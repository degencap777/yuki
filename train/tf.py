...
BOARD_ACTION_N = BOARD_SQUARE_SIZE + 1
SIDE_PLANE = 1
PLANES_N = 2
...
class TFProcess:
    def __init__(self, next_batch):
        ...
        self.x = next_batch[0]#tf.placeholder(tf.float32, [None, 4, 8 * 8])
        self.y_ = next_batch[1] # tf.placeholder(tf.float32, [None, 65])
    ...
    def construct_net(self, planes):
        ...
        # NCHW format
        # batch, 4 channels, 8 x 8
        x_planes = tf.reshape(planes,[-1,PLANES_N+2,BOARD_SIZE,BOARD_SIZE])

        # Input convolution
        flow = self.conv_block(x_planes, filter_size=3,
                               input_channels=PLANES_N + 2,
        ...
        W_fc1 = weight_variable([2 * 8 * 8, (8 * 8) + 1])
        b_fc1 = bias_variable([(8 * 8) + 1])
